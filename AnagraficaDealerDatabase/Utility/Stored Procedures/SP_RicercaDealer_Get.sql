CREATE PROCEDURE [Utility].[SP_RicercaDealer_Get] 
		@vXML AS XML,
		@Utente nvarchar(20)
AS
BEGIN
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				SET @WhereCondictions1 = @WhereCondictions1 + @ID + ' LIKE ''%' + replace(@value,'$$','&') + '%'' '
			END
			IF @IDTipoDealer like '%2%'
			BEGIN
				SET @WhereCondictions2 = @WhereCondictions2 + @ID + ' LIKE ''%' + replace(@value,'$$','&') + '%'' '
			END
			IF @IDTipoDealer like '%3%'
			BEGIN
				SET @WhereCondictions3 = @WhereCondictions3 + @ID + ' LIKE ''%' + replace(@value,'$$','&') + '%'' '
			END
			IF @IDTipoDealer like '%4%'
			BEGIN
				SET @WhereCondictions4 = @WhereCondictions4 + @ID + ' LIKE ''%' + replace(@value,'$$','&') + '%'' '
			END
			IF @IDTipoDealer like '%5%'
			BEGIN
				SET @WhereCondictions5 = @WhereCondictions5 + @ID + ' LIKE ''%' + replace(@value,'$$','&') + '%'' '
			END
		END
		IF @type like 'combo'
		BEGIN
		...
		BEGIN
			...
			BEGIN
				BEGIN
					SET @WhereCondictions1 = @WhereCondictions1 + ' (CONVERT(date, CONVERT(varchar(10), ' + @ID + ', 102)) <> NULL '
				END
				IF @IDTipoDealer like '%2%'
				BEGIN
					SET @WhereCondictions2 = @WhereCondictions2 + ' (CONVERT(date, CONVERT(varchar(10), ' + @ID + ', 102)) <> NULL '
				END
				IF @IDTipoDealer like '%3%'
				BEGIN
					SET @WhereCondictions3 = @WhereCondictions3 + ' (CONVERT(date, CONVERT(varchar(10), ' + @ID + ', 102)) <> NULL '
				END
				IF @IDTipoDealer like '%4%'
				BEGIN
					SET @WhereCondictions4 = @WhereCondictions4 + ' (CONVERT(date, CONVERT(varchar(10), ' + @ID + ', 102)) <> NULL '
				END
				IF @IDTipoDealer like '%5%'
				BEGIN
					SET @WhereCondictions5 = @WhereCondictions5 + ' (CONVERT(date, CONVERT(varchar(10), ' + @ID + ', 102)) <> NULL '
				END
			END
...
BEGIN
	SET @Campi = 'CampoDefault1, CampoDefault2, CampoDefault3'
END
...
BEGIN
	...
	SET @Query = 'SELECT ' + @Campi + ' FROM Dealer.View_Committenti ' + @WhereCondictions1
END
...
BEGIN
	...
	BEGIN
		...
		SET @Query = @Query + ' UNION SELECT ' + @Campi + ' FROM Dealer.View_PosPod ' + @WhereCondictions2
	END
	ELSE
	BEGIN
		--SET @Query = 'SELECT ' + @Campi + ' FROM Dealer.View_PosPod ' + SUBSTRING(@WhereCondictions2,1,LEN(@WhereCondictions2)-4) 
		SET @Query = 'SELECT ' + @Campi + ' FROM Dealer.View_PosPod ' + @WhereCondictions2
	END
END
...
BEGIN
	...
	BEGIN
		...
		SET @Query = @Query + ' UNION SELECT ' + @Campi + ' FROM Dealer.View_Point ' + @WhereCondictions3 
	END
	ELSE
	BEGIN
		--SET @Query = 'SELECT ' + @Campi + ' FROM Dealer.View_Point ' + SUBSTRING(@WhereCondictions3,1,LEN(@WhereCondictions3)-4)
		SET @Query = 'SELECT ' + @Campi + ' FROM Dealer.View_Point ' + @WhereCondictions3
	END
END
...
BEGIN
	...
	BEGIN
		...
		SET @Query = @Query + ' UNION SELECT ' + @Campi + ' FROM Dealer.View_AgentiCommittenti ' + @WhereCondictions4 
	END
	ELSE
	BEGIN
		--SET @Query = 'SELECT ' + @Campi + ' FROM Dealer.View_AgentiCommittenti ' + SUBSTRING(@WhereCondictions4,1,LEN(@WhereCondictions4)-4) 
		SET @Query = 'SELECT ' + @Campi + ' FROM Dealer.View_AgentiCommittenti ' + @WhereCondictions4 
	END
END
...
BEGIN
	...
	BEGIN
		...
		SET @Query = @Query + ' UNION SELECT ' + @Campi + ' FROM Dealer.View_SubAgenti ' + @WhereCondictions5
	END
	ELSE
	BEGIN
		--SET @Query = 'SELECT ' + @Campi + ' FROM Dealer.View_SubAgenti ' + SUBSTRING(@WhereCondictions5,1,LEN(@WhereCondictions5)-4)
		SET @Query = 'SELECT ' + @Campi + ' FROM Dealer.View_SubAgenti ' + @WhereCondictions5 
	END
END

--PRINT(@Query)

set @Query = @Query + ' Order By Codice,RagioneSociale'
