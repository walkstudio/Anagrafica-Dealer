CREATE PROCEDURE [Import].[SP_ImportCommittenti] 
AS
BEGIN
	SET NOCOUNT ON;
	...
	BEGIN TRANSACTION
		...
		BEGIN TRY 
			...
			SELECT C.Indirizzo, C.PIVA, C.Telefono, C.Fax, @ChangeDate, @SystemUserID, @IDTipoDealer, C.IDStato, C.Codice, C.Funzione
			...
			INSERT INTO InfoGenerali.InfoCommittenti ( IDDealer, BOL, DataAperturaCodice, DataChiusuraCodice )
			...
			SET InfoGenerali.InfoCommittenti.BOL = C.BOL,
					InfoGenerali.InfoCommittenti.DataAperturaCodice = C.DataAttivazione,
					InfoGenerali.InfoCommittenti.DataChiusuraCodice = C.DataDisattivazione
			FROM Import.Committente C
	...
	BEGIN TRANSACTION
		...
		BEGIN TRY 
			-- Logically Delete Committente
			UPDATE Dealer.Dealer SET Attivo = 0
			FROM Dealer.Dealer D INNER JOIN Import.CommittenteToHide C ON
				D.IDDealer = C.IDDealer
